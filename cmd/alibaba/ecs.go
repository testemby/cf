...
var ecsExecCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		if lhost != "" && lport == "" {
			...
			aliecs2.ECSExec(command, commandFile, scriptType, ecsExecSpecifiedInstanceID, ecsExecRegion, batchCommand, userData, metaDataSTSToken, ecsFlushCache, lhost, lport, timeOut, ecsExecAllRegions, userDataBackdoor, imageShare)
		}
	},
}